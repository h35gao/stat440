...
#' \preformatted{
#' pi(gamma,lambda) = pi(eta,lambda) * gamma/lambda^(gamma+1).
#' }
#'
...
#' Marginalized Metropolized IID sampling for the Weibull posterior parameter distribution.
#'
#' @param nsamples Number of posterior iterations (not including burn-in).
#' @param y Vector of survival times (non-negative).
#' @param gamma.rng Range for mode-finding algorithm on marginal posterior \code{p(gamma | y)}.
#' @param burn Integer number of initial iterations to discard as burn-in.  The default value is \code{min(nsamples/10, 1000)}.
#' @param acc.out Logical; whether or not to return the proportion of accepted proposals.
#' @return A \code{nsamples x 2} matrix of posterior draws, or a list with elements \code{Theta} and \code{accept}, the first of which are the posterior draws, the second being the proportion of accepted proposals.
#' @details The prior distribution is flat on \code{(gamma, eta)}, i.e., \code{pi(gamma,lambda) ~ 1}, where \code{eta = 1/lambda^gamma}.  Note that by change-of-variables, this results in the original scale prior
#' \preformatted{
#' pi(gamma,lambda) ~ gamma/lambda^(gamma+1).
#' }
weibull.mmi <- function(nsamples, y, gamma.rng, burn, acc.out = FALSE) {
  if(missing(burn)) burn <- min(floor(nsamples/10), 1e3) # default burn-in
...
#'
#' @param gseq Grid of \code{gamma} values.
#' @param lseq Grid of \code{lambda} values.
#' @param lpmat Matrix of log-posterior density evaluations on the grid determined by \code{gseq} and \code{lseq}.
#' @param alpha Level for credible intervals.
#' @return A list with elements:
#' \describe{
#'   \item{\code{gseq},\code{lseq}}{Sequences of \code{gamma} and \code{lambda} values.}
#'   \item{\code{gldens}}{Unnormalized joint posterior density \code{p(gamma, lambda | y)}.}
#'   \item{\code{gdens},\code{ldens}}{Normalized marginal posterior densities \code{p(gamma | y)} and \code{p(lambda | y)}.}
#'   \item{\code{gmean},\code{lmean}}{Posterior means.}
#'   \item{\code{gCI},\code{lCI}}{\code{alpha}-level credible intervals.}
#' }
